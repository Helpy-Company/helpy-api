...
class FakeContractorsRepository implements IContractorRepository {
  ...
  public async findByEmail(email: string): Promise<Contractor | undefined> {
    const findContractor = this.contractors.find((contractor) => contractor.email === email);

    return findContractor;
  }

  public async findById(id: string): Promise<Contractor | undefined> {
    const findContractor = this.contractors.find((contractor) => contractor.id === id);

    return findContractor;
  }

  public async create(contractorData: ICreateContractorsDTO): Promise<Contractor> {
  ...
  public async save(contractor: Contractor): Promise<Contractor> {
    const findIndex = this.contractors.findIndex((findContractor) => findContractor.id === contractor.id);
